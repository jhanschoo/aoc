...
int main() {
	...
	vector<coords> translations{{0, 0, 0}};
	located.PB(*first);
	...
	for (const auto &v : translations) {
		for (const auto &w : translations) {
			max_dist = max(max_dist, L1(sub(v, w)));
		}
	}
	cout << "Max distance: " << max_dist << enl;
}
