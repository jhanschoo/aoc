...
int main() {
    ...
    auto orig_numbers = ranges::views::zip(
        ...
        return number_t{idx, val, dummy};
    }) | ranges::to<std::vector>();
    for (auto &&num: orig_numbers) {
        nums_orig_idx.emplace(num);
    }
    auto sg_numbers_sz = static_cast<int>(numbers.size());
    for (auto &[orig_idx, raw_value, dummy_]: orig_numbers) {
        if (raw_value == 0) {
            continue;
        }
...
//        std::cout << std::endl;
    ...
    for (auto delta : {1000, 2000, 3000}) {
        sum += nums_rank_idx.nth((zero_rank + delta) % numbers.size())->value;
    }
