...
int main() {
    ...
    auto regimes = std::array{
        ...
        std::tuple{
            num_rows, // num_maj
            num_cols, // num_min
            +[](std::size_t num_rows, std::size_t num_cols, std::size_t maj, std::size_t min) { return maj * num_cols + min; },
        ...
        std::tuple{
            num_rows,
            num_cols,
            +[](std::size_t num_rows, std::size_t num_cols, std::size_t maj, std::size_t min) { return maj * num_cols + (num_cols - min - 1); },
            std::valarray(std::size_t{}, cells)
        },
        // col-wise utd
        std::tuple{
            num_cols,
            num_rows,
            +[](std::size_t num_rows, std::size_t num_cols, std::size_t maj, std::size_t min) { return maj + min * num_cols; },
            std::valarray(std::size_t{}, cells)
        },
        // col-wise dtu
        std::tuple{
            num_cols,
            num_rows,
            +[](std::size_t num_rows, std::size_t num_cols, std::size_t maj, std::size_t min) { return maj + (num_rows - min - 1) * num_cols; },
            std::valarray(std::size_t{}, cells)
        },
    };
	...
	std::cout << (std::get<3>(regimes[0]) * std::get<3>(regimes[1]) * std::get<3>(regimes[2]) * std::get<3>(regimes[3])).max() << std::endl;
}
